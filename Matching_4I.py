...
import jellyfish
# --- Configuration ---
INPUT_JSON = "scraper_results_prompt_CH.json"
OUTPUT_CSV = "analysis_results_PromptX.csv"
# ---------------------
...
def create_llm_prompt(company_data: Dict[str, Any], scraped_content: str) -> str:
    ...
    #sic_code_str = str({company_data.get('sic_code_1') + " (" + company_data.get('sic_code_2') + ")"})
    # 1. Get the values, ensuring we don't crash on None
    code_1 = company_data.get('sic_code_1')
    code_2 = company_data.get('sic_code_2')

# 2. Filter out None or empty values and convert to string (in case they are ints)
    valid_codes = [str(c) for c in [code_1, code_2] if c]

# 3. Join them with a comma, or default to "N/A" if both are empty
    sic_code_str = ", ".join(valid_codes) if valid_codes else "N/A"
    return f"""
